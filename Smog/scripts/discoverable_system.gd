...
func _parse_json() -> void:
	...
	for room_name in rooms:
		...
		for key in room:
			discoverables[room_name].display_name = room[key].display_name
			discoverables[room_name].text = room[key].text
			discoverables[room_name].type = _string_type(room[key].type)
			discoverables[room_name].texture_path = room[key].texture_path
			...
			if room[key].type != "IMAGE":
				continue
			discoverables[room_name].image_path = room[key].image_path
