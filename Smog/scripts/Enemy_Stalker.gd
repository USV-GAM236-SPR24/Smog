...
func _physics_process(delta):
	...
	if draining:
		...
		if (velocity.x > 0 && (Ranged == false) && (draining == true) && ((abs(velocity.y) < abs(velocity.x)))): #right MeleeStance
			$Animation.play("Atk LR")
			$Animation.flip_h = true
		if (velocity.x <= 0  && (Ranged == false) && (draining == true) && ((abs(velocity.y) < abs(velocity.x)))): #left MeleeStance
			$Animation.play("Atk LR")
			$Animation.flip_h = false
		if (velocity.y >= 0  && (Ranged == false) && (draining == true) && ((abs(velocity.y) > abs(velocity.x)))): #down Meleestance
			$Animation.play("Atk D")
			$Animation.flip_h = false
		if (velocity.y < 0  && (Ranged == false) && (draining == true) && ((abs(velocity.y) > abs(velocity.x)))):
	...
	if (velocity.x > 0 && (Ranged == false) && (draining == false)): #right MeleeStance
		$Animation.play("Melee LR")
		$Animation.flip_h = true
	if (velocity.x <= 0  && (Ranged == false) && (draining == false)): #left MeleeStance
		$Animation.play("Melee LR")
		$Animation.flip_h = false
